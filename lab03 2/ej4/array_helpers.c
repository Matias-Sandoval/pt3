...
void array_from_file(WeatherTable array, const char *filepath) {
    ...
    while (!feof(file)) {
        int res = fscanf(file, " %u %u %u %u %u %u %u %u %u ", &k_year, &k_month, &k_day, &_average_temp, &_max_temp, &_min_temp, &_pressure, &_moisture, _rainfall);
        if (res != 3) {
        ...
        Weather weather;
        weather._average_temp = _average_temp;
        weather._max_temp = _max_temp;
        weather._min_temp = _min_temp;
        weather._pressure = _pressure;
        weather._moisture = _moisture;
        weather._rainfall = _rainfall;
